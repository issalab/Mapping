...
# ------------------------------------------------------------
nf = 20
for nc in np.arange(nf,1,-1):

    for imag_feat_ratio in [1, 2, 4]:  #4, 8, 16, 64, 128
        ...
        pickle.dump(data_list, open(resultdir + 'unit_test_%s_%s_%s, ni%d_nf%d_nt%d_collinearity%s_%s_SB%s_VariousSd%s_noisymap%s_statsfromHvM%s_%dcmp.pickle' % (
                reg_methods, reg_params_list, PCA_ncomponents_list, ni, nf, nt, Collinearity, noise_dist, spearman_brown, various_unit_stds, noisy_map,stats_from_data,nc), 'wb'))